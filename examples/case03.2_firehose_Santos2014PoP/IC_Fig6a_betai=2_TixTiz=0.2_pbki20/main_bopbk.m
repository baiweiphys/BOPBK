...
% @Date: 2026-03-30
% @Modification: 
%   Bai Wei, 2026.05.02: Updated BM to MK (retains BM features).

% dos Santos et al., Phys. Plasmas 21, 112102 (2014);
% "Ion firehose instability in plasmas with plasma particles described by
%  product bi-kappa distributions". doi:10.1063/1.4900766
...
B0 = 9.7251e-5; % background magnetic field in z direction 
nk = 100;
kk0 = linspace(5.0e-2,1.8,nk);
EPS0 = 1.0e-3; % for integrals S1,S2,...,S9 when \lambda_s \rightarrow 0
...
% di = vA_p/wcs(ion_ind); % ion inertial length di=vA/Omega_i
norm_omega = abs(wcs(ion_ind));
...
save(fname,'deg','theta','S','par','deg','kk0','kk','ww','www','wps','wcs','lambdaDs','rhocs','Ns','runtime');
