...
nk = 160;
kk0 = linspace(5.0e-2,2.5,nk);
EPS0 = 1.0e-3; % for integrals S1,S2,...,S6 when \lambda_s \rightarrow 0
...
% plot(kk0,real_w(:,rootNo)/wps(1),'k.','markersize',15);
xlabel('ck/\omega_{pe}');
ylabel('\omega_r/\omega_{pe}');
...
% plot(kk0,imag_w(:,rootNo)/wps(1),'b.-','markersize',15);
xlabel('ck/\omega_{pe}');
ylabel('\gamma/\omega_{pe}');
grid on;
