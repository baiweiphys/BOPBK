...
if S_bm==0
    tmp_pbk = getIndexOfBlkMatrix_pbk(Ns_pbk,kappasz_pbk,3);
    len = tmp_pbk(end) + 6;
elseif S_pbk==0
    tmp_bm = getIndexOfBlkMatrix_maxwell(Ns_bm,J,3);
    len = tmp_bm(end) + 9;
else
    tmp_mixed = getIndexOfBlkMatrix_mixed(Ns_pbk,Ns_bm,kappasz_pbk,J,6);
    len = tmp_mixed(end) + 9;
end
