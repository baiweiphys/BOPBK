function [w,eigVec] = solver_mixed_pbk2(kx,kz,theta,plasmaParams,sp,norm_omega,EPS0)
...
% model using the mixed distribution in both loss-cone Maxwellian-kappa (MK) 
% and PBK (type 2) plasmas, the transverse wave number kx and longitudinal 
% wave number kz are specified.
...
%                        otherwise uses MK.
%   Bai Wei, 2026.04.05: update solver to PBK type 2.
...
%% for PBK2 distribution plasma

% common/PBK type 1 or 2
csn = @(s,n) pbk_csn(s,n,kz,kappasz_pbk,vtsz_pbk2,wcs_pbk,us0_pbk,norm_omega);

% PBK type 2
b1snl = @(s,n,l) pbk2_b1snl(s,n,l,kz,kappasz_pbk,kappasx_pbk,vtsz_pbk2,sgms_pbk,wcs_pbk,lambdas_pbk,norm_omega,EPS0);
b2snl = @(s,n,l) pbk2_b2snl(s,n,l,kz,kappasz_pbk,kappasx_pbk,vtsz_pbk2,vtsx_pbk2,sgms_pbk,lambdas_pbk,norm_omega,EPS0);
b3snl = @(s,n,l) pbk2_b3snl(s,n,l,kz,kappasz_pbk,kappasx_pbk,vtsz_pbk2,sgms_pbk,wcs_pbk,lambdas_pbk,norm_omega,EPS0);
b4snl = @(s,n,l) pbk2_b4snl(s,n,l,kz,kappasz_pbk,kappasx_pbk,vtsz_pbk2,vtsx_pbk2,sgms_pbk,lambdas_pbk,norm_omega,EPS0);
b5snl = @(s,n,l) pbk2_b5snl(s,n,l,kz,kappasz_pbk,kappasx_pbk,vtsz_pbk2,sgms_pbk,wcs_pbk,lambdas_pbk,norm_omega,EPS0);
b6snl = @(s,n,l) pbk2_b6snl(s,n,l,kz,kappasz_pbk,kappasx_pbk,vtsz_pbk2,vtsx_pbk2,sgms_pbk,lambdas_pbk,norm_omega,EPS0);
...
% PBK type 2
b12snj = @(s,n,jj) mk2_b12snj(s,n,jj,kz,kappasx,kappasx_th,J_opt,vtsz_mk,Tsz_mk,Tsx_mk,sgms_mk,wcs_mk,lambdas_mk,norm_omega,EPS0);
b34snj = @(s,n,jj) mk2_b34snj(s,n,jj,kz,kappasx,kappasx_th,J_opt,vtsz_mk,Tsz_mk,Tsx_mk,sgms_mk,wcs_mk,lambdas_mk,norm_omega,EPS0);
b56snj = @(s,n,jj) mk2_b56snj(s,n,jj,kz,kappasx,kappasx_th,J_opt,vtsz_mk,Tsz_mk,Tsx_mk,sgms_mk,wcs_mk,lambdas_mk,norm_omega,EPS0);
...
% PBK type 2
Mx_pbk = Mxy_pbk2_mixed(S_pbk2,Ns_pbk,Ns_mk,kappasz_pbk,Jpole,csn, ...
                        bx11snl,bx21snl,bx31snl,bx12snl,bx22snl,bx32snl, ...
                        bx33snl,bx10_pbk,1,5,4,3,5); % Matrix No.1

% PBK type 2
My_pbk = Mxy_pbk2_mixed(S_pbk2,Ns_pbk,Ns_mk,kappasz_pbk,Jpole,csn, ...
                        by11snl,by21snl,by31snl,by12snl,by22snl,by32snl, ...
                        by33snl,by20_pbk,2,5,4,3,4); % Matrix No.2

% PBK type 2
Mz_pbk = Mz_pbk2_mixed(S_pbk2,Ns_pbk,Ns_mk,kappasz_pbk,Jpole,csn, ...
...
% PBK type 2
Mx_mk = M_mk_mixed_pbk2(S_mk,Ns_pbk,Ns_mk,kappasz_pbk,Jpole,csnj,bx1snj,bx2snj,bx3snj,4,5,4,3); % Matrix No.4
My_mk = M_mk_mixed_pbk2(S_mk,Ns_pbk,Ns_mk,kappasz_pbk,Jpole,csnj,by1snj,by2snj,by3snj,5,5,4,3); % Matrix No.5
Mz_mk = M_mk_mixed_pbk2(S_mk,Ns_pbk,Ns_mk,kappasz_pbk,Jpole,csnj,bz1snj,bz2snj,bz3snj,6,5,4,3); % Matrix No.6

%% Step 3: Assemble matrix based on the maxied distribution of PBK2 and MK plasmas.
M_pbk = [Mx_pbk; My_pbk; Mz_pbk];
% disp(['size(Mx_pbk2) = ',num2str(size(Mx_pbk2))]);
% disp(['size(My_pbk2) = ',num2str(size(My_pbk2))]);
% disp(['size(Mz_pbk2) = ',num2str(size(Mz_pbk2))]);
...
%% Step 5.1: The perturbed currents of Maxwell's equations for the PBK2 plasmas                 
%  Note (2026.04.02): where J_{x,y,z} is J_{x,y,z}/norm_omega
% PBK type 2
idx_Jx_pbk2 = getIndexOfBlkMatrix_mixed_pbk2(Ns_pbk,Ns_mk,kappasz_pbk,Jpole,1);
M(end-5,idx_Jx_pbk2(end)) = M(end-5,idx_Jx_pbk2(end)) - 1i/epsilon0; % for the second item of Ex

% PBK type 2
idx_Jy_pbk2 = getIndexOfBlkMatrix_mixed_pbk2(Ns_pbk,Ns_mk,kappasz_pbk,Jpole,2);
M(end-4,idx_Jy_pbk2(end)) = M(end-4,idx_Jy_pbk2(end)) - 1i/epsilon0; % for the third item of Ey

% PBK type 2
idx_Jz_pbk2 = getIndexOfBlkMatrix_mixed_pbk2(Ns_pbk,Ns_mk,kappasz_pbk,Jpole,3);
M(end-3,idx_Jz_pbk2(end)) = M(end-3,idx_Jz_pbk2(end)) - 1i/epsilon0; % for the second item of Ez

%% Step 5.2: The perturbed currents of Maxwell's equations for the Maxwellian-kappa (MK) plasmas
%  Note (2026.04.02): where J_{x,y,z} is J_{x,y,z}/norm_omega
% PBK type 2
idx_Jx_mk = getIndexOfBlkMatrix_mixed_pbk2(Ns_pbk,Ns_mk,kappasz_pbk,Jpole,4);
M(end-5,end-1) = M(end-5,end-1) + c2*kz/norm_omega; % for the first item of Ex
M(end-5,end-8) = M(end-5,end-8) - 1i/epsilon0; % for dJx1_mk
M(end-5,idx_Jx_mk(1:end-1)) = M(end-5,idx_Jx_mk(1:end-1)) - 1i/epsilon0; % for dJx2_mk

% PBK type 2
idx_Jy_mk = getIndexOfBlkMatrix_mixed_pbk2(Ns_pbk,Ns_mk,kappasz_pbk,Jpole,5);
...
M(end-4,idx_Jy_mk(1:end-1)) = M(end-4,idx_Jy_mk(1:end-1)) - 1i/epsilon0; % for dJy2_mk

% PBK type 2
idx_Jz_mk = getIndexOfBlkMatrix_mixed_pbk2(Ns_pbk,Ns_mk,kappasz_pbk,Jpole,6);
M(end-3,end-1) = M(end-3,end-1) - c2*kx/norm_omega;  % for the first item of Ez
M(end-3,end-6) = M(end-3,end-6) - 1i/epsilon0; % for dJz1_mk
M(end-3,idx_Jz_mk(1:end-1)) = M(end-3,idx_Jz_mk(1:end-1)) - 1i/epsilon0; % for dJz2_mk
