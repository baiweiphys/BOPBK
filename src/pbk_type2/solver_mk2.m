function  [w,eigVec] = solver_mk2(kx,kz,theta,plasmaParams,sp,norm_omega,EPS0)
% SOLVER_MK To solve for the wave mode roots in the MK solver. 
% @Description: To solve for the wave mode roots of the oblique plasma wave 
% model using the loss-cone Maxwellian-kappa (MK, PBK type 2) distribution, the transverse wave 
% number kx and longitudinal wave number kz are specified.
% @Filename: solver_mk.m
...
%                        otherwise uses MK.
%   Bai Wei, 2026.04.05: update solver to MK (PBK type 2).
...
% PBK type 2
b12snj = @(s,n,jj) mk2_b12snj(s,n,jj,kz,kappasx,kappasx_th,J_opt,vtsz,Tsz,Tsx,sgms,wcs,lambdas,norm_omega,EPS0);
b34snj = @(s,n,jj) mk2_b34snj(s,n,jj,kz,kappasx,kappasx_th,J_opt,vtsz,Tsz,Tsx,sgms,wcs,lambdas,norm_omega,EPS0);
b56snj = @(s,n,jj) mk2_b56snj(s,n,jj,kz,kappasx,kappasx_th,J_opt,vtsz,Tsz,Tsx,sgms,wcs,lambdas,norm_omega,EPS0);
...
% for y-component (common/PBK type 1 or 2common/PBK type 1 or 2)
...
% common/PBK type 1 or 2
idx_Jx_mk = getIndexOfBlkMatrix_mk(Ns,Jpole,1);
M(end-5,end-1) = M(end-5,end-1) + c2*kz/norm_omega; % for the first item of Ex
M(end-5,end-8) = M(end-5,end-8) - 1i/epsilon0; % for dJx1
M(end-5,idx_Jx_mk(1:end-1)) = M(end-5,idx_Jx_mk(1:end-1)) - 1i/epsilon0; % for dJx2

% common/PBK type 1 or 2
idx_Jy_mk = getIndexOfBlkMatrix_mk(Ns,Jpole,2);
...
M(end-4,idx_Jy_mk(1:end-1)) = M(end-4,idx_Jy_mk(1:end-1)) - 1i/epsilon0; % for dJy2

% common/PBK type 1 or 2
idx_Jz_mk = getIndexOfBlkMatrix_mk(Ns,Jpole,3);
M(end-3,end-1) = M(end-3,end-1) - c2*kx/norm_omega;  % for the first item of Ez
M(end-3,end-6) = M(end-3,end-6) - 1i/epsilon0; % for dJz1
M(end-3,idx_Jz_mk(1:end-1)) = M(end-3,idx_Jz_mk(1:end-1)) - 1i/epsilon0; % for dJz2
%
